            ...
            <div className="flex-grow">
               <Frame>
                  <table className="w-full">
                     <tbody>
                        {data.category_data.attributes.map((catAttr, index) => {
                           const founded = data.attributes_data.find(
                              (attr) =>
                                 attr.attribute_data.attribute_ascii ==
                                 catAttr.attribute_ascii
                           );

                           return (
                              <tr
                                 className="even:bg-[#f1f1f1] rounded-[6px]"
                                 key={index}
                              >
                                 <td className="w-[40%] px-[4px] py-[2px]">
                                    {catAttr.attribute}
                                 </td>
